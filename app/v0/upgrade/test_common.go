...
import (
	...
	staking_types "github.com/netcloth/netcloth-chain/app/v0/staking/types"
...
// CreateTestInput create test input
// nolint
func CreateTestInput(t *testing.T, initPower int64) (ctx sdk.Context, keeper Keeper, stakingKeeper staking.Keeper, paramsKeeper params.Keeper) {
	...
	stakingKeeper = staking.NewKeeper(cdc, keys[staking.StoreKey], tkeys[staking.TStoreKey], supplyKeeper, paramsKeeper.Subspace(staking.DefaultParamspace))
	stakingKeeper.SetParams(ctx, staking_types.DefaultParams())
	...
	protocolKeeper := sdk.NewProtocolKeeper(keys["main"])
	keeper = NewKeeper(
		cdc,
		keys[types.StoreKey],
		protocolKeeper,
