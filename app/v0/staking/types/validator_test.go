...
func TestAddTokensValidatorBonded(t *testing.T) {
	...
	validator, delShares := validator.AddTokensFromDel(sdk.NewInt(10), true)
...
func TestRemoveDelShares(t *testing.T) {
	valA := Validator{
		...
		ConsPubKey:      pk1,
		Status:          sdk.Bonded,
		SelfDelegation:  sdk.NewInt(100).ToDec(),
		Tokens:          sdk.NewInt(100),
		DelegatorShares: sdk.NewDec(100),
	}

	// Remove delegator shares
	valB, coinsB := valA.RemoveDelShares(sdk.NewDec(10), true)
	...
	validator := Validator{
		...
		ConsPubKey:      pk1,
		Status:          sdk.Bonded,
		SelfDelegation:  delShares,
		Tokens:          poolTokens,
		DelegatorShares: delShares,
	}

	shares := sdk.NewDec(29)
	_, tokens := validator.RemoveDelShares(shares, true)
...
func TestAddTokensFromDel(t *testing.T) {
	...
	validator, shares := validator.AddTokensFromDel(sdk.NewInt(6), true)
	...
	validator, shares = validator.AddTokensFromDel(sdk.NewInt(3), true)
...
func TestUpdateStatus(t *testing.T) {
	validator := NewValidator(sdk.ValAddress(pk1.Address().Bytes()), pk1, Description{})
	validator, _ = validator.AddTokensFromDel(sdk.NewInt(100), true)
	require.Equal(t, sdk.Unbonded, validator.Status)
...
func TestPossibleOverflow(t *testing.T) {
	...
	newValidator, _ := validator.AddTokensFromDel(sdk.NewInt(71), true)
